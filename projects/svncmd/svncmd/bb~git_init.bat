...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%SVNCMD_ROOT.SVN.REPOROOT%%" --stdlayout || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT2.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT2.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%SVNCMD_ROOT.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%SVNCMD_ROOT.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url scripts > nul 2> nul && call :CMD git remote set-url scripts "%%SVNCMD_SCRIPTS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add scripts "%%SVNCMD_SCRIPTS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools > nul 2> nul && call :CMD git remote set-url tools "%%SVNCMD_TOOLS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add tools "%%SVNCMD_TOOLS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url external-tools > nul 2> nul && call :CMD git remote set-url external-tools "%%EXTERNAL_TOOLS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add external-tools "%%EXTERNAL_TOOLS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
