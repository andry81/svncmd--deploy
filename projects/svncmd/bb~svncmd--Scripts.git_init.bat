...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%SVNCMD_SCRIPTS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%SVNCMD_SCRIPTS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  popd
)
