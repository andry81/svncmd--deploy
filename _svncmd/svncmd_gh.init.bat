...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%SVNCMD_ROOT.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%SVNCMD_ROOT.GIT.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url scripts > nul 2> nul && call :CMD git remote set-url scripts "%%SVNCMD_SCRIPTS.GIT.ORIGIN%%"
  ) || call :CMD git remote add scripts "%%SVNCMD_SCRIPTS.GIT.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools > nul 2> nul && call :CMD git remote set-url tools "%%SVNCMD_TOOLS.GIT.ORIGIN%%"
  ) || call :CMD git remote add tools "%%SVNCMD_TOOLS.GIT.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url external-tools > nul 2> nul && call :CMD git remote set-url external-tools "%%EXTERNAL_TOOLS.GIT.ORIGIN%%"
  ) || call :CMD git remote add external-tools "%%EXTERNAL_TOOLS.GIT.ORIGIN%%" || ( popd & goto EXIT )
